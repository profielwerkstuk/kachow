...
import { Visualiser } from "./classes/Visualiser";
import { Car as _Car } from "./classes/Car";
...
export function load() {
	...
	onSnapshot(collection(firebase, "genomes"), (snapshot: { docChanges: () => any[]; }) => {
		snapshot.docChanges().forEach((change) => {
			if (change.type === "added") {
				...
				const Car = new _Car(carSpawnPoint, carWidth, carHeight, tileSize, carViewingDistance, data.carName);
